@php
    ...
    if (isset($h1AgeBreakdown)) {
        $totalForms = array_sum(array_column(array_column($h1AgeBreakdown, 'total'), 'count'));
    } elseif (isset($clientSexBreakdownJanToJuneJanToJune)) {
        $totalForms = array_sum(array_map(function ($counts) {
            return $counts['Internal'] + $counts['External'];
        }, $clientSexBreakdownJanToJuneJanToJune));
    } elseif (isset($h1MunicipalityBreakdown)) {
        $totalForms = array_sum(array_column(array_column($h1MunicipalityBreakdown, 'total'), 'count'));
    } elseif (isset($h1CategoryBreakdown)) {
        $totalForms = array_sum(array_column(array_column($h1CategoryBreakdown, 'total'), 'count'));
    }
@endphp
...
<div class="card" style="margin-top: 20px; margin-left: 20px; border-radius: 10px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); background-color: #fff;">
    ...
    <div class="card-body">
        <table class="table table-bordered">
            ...
            <tbody>
                @foreach ($h1AgeBreakdown as $ageRange => $data)
                    <tr>
                        <td>{{ $ageRange }}</td>
                ...
                <tr>
                    ...
                    <td>
                        <strong>
                            {{ array_sum(array_column(array_column($h1AgeBreakdown, 'external'), 'count')) }} 
                            ({{ number_format(array_sum(array_column(array_column($h1AgeBreakdown, 'external'), 'percentage')), 2) }}%)
                        </strong>
                    </td>
                    <td>
                        <strong>
                            {{ array_sum(array_column(array_column($h1AgeBreakdown, 'internal'), 'count')) }} 
                            ({{ number_format(array_sum(array_column(array_column($h1AgeBreakdown, 'internal'), 'percentage')), 2) }}%)
                        </strong>
                    </td>
                    <td>
                        <strong>
                            {{ array_sum(array_column(array_column($h1AgeBreakdown, 'total'), 'count')) }} 
                            ({{ number_format(array_sum(array_column(array_column($h1AgeBreakdown, 'total'), 'percentage')), 2) }}%)
                        </strong>
                    </td>
                </tr>
            </tbody>
    ...
    <div class="card-body">
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                @foreach ($h1MunicipalityBreakdown as $municipality => $counts)
                    @php
...
<div class="card" style="margin-top: 20px; margin-left: 20px; border-radius: 10px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); background-color: #fff;">
    ...
    <div class="card-body">
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                @foreach ($h1CategoryBreakdown as $category => $counts)
                    @php
    ...
    <!-- Card for Citizen's Charter Awareness - January to June -->
    <div class="card" style="margin-top: 20px; margin-left: 20px; border-radius: 10px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); background-color: #fff;">
                    <div class="card-header">
