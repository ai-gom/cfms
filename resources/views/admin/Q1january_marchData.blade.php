@php
    ...
    if (isset($q1AgeBreakdown)) {
        ...
        $totalForms = array_sum(array_column(array_column($q1CategoryBreakdown, 'total'), 'count'));
    }
@endphp
...
<div class="card" style="margin-top: 20px; margin-left: 20px; border-radius: 10px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); background-color: #fff;">
    ...
    <div class="card-body">
        <table class="table table-bordered">
            ...
            <tbody>
                @foreach ($q1AgeBreakdown as $ageRange => $data)
                    <tr>
                        <td>{{ $ageRange }}</td>
                ...
                <tr>
                    ...
                    <td>
                        <strong>
                            {{ array_sum(array_column(array_column($q1AgeBreakdown, 'external'), 'count')) }} 
                            ({{ number_format(array_sum(array_column(array_column($q1AgeBreakdown, 'external'), 'percentage')), 2) }}%)
                        </strong>
                    </td>
                    <td>
                        <strong>
                            {{ array_sum(array_column(array_column($q1AgeBreakdown, 'internal'), 'count')) }} 
                            ({{ number_format(array_sum(array_column(array_column($q1AgeBreakdown, 'internal'), 'percentage')), 2) }}%)
                        </strong>
                    </td>
                    <td>
                        <strong>
                            {{ array_sum(array_column(array_column($q1AgeBreakdown, 'total'), 'count')) }} 
                            ({{ number_format(array_sum(array_column(array_column($q1AgeBreakdown, 'total'), 'percentage')), 2) }}%)
                        </strong>
                    </td>
                </tr>
            </tbody>
    ...
    <div class="card-body">
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                @foreach ($q1MunicipalityBreakdown as $municipality => $counts)
                    @php
    ...
    <div class="card-body">
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                @foreach ($q1CategoryBreakdown as $category => $counts)
                    @php
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                <tr>
                    ...
                    <td>{{ $januaryToMarchTotals['strongly_agree'] }}</td>
                    <td>{{ $januaryToMarchTotals['agree'] }}</td>
                    <td>{{ $januaryToMarchTotals['neither'] }}</td>
                    <td>{{ $januaryToMarchTotals['disagree'] }}</td>
                    <td>{{ $januaryToMarchTotals['strongly_disagree'] }}</td>
                    <td>{{ $januaryToMarchTotals['na'] }}</td>
                    <td>{{ $januaryToMarchTotals['total_responses'] }}</td>
                    <td>{{ number_format($januaryToMarchTotals['average_overall_score'], 2) }}%</td>
                </tr>
            </tbody>
        </table>
    </div>
    ...
    <div class="card-body">
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                @endphp
                @foreach($januaryToMarchExpectationsBreakdown as $field => $breakdown)
                    @php
