@php
    ...
    if (isset($q4AgeBreakdown)) {
        $totalForms = array_sum(array_column(array_column($q4AgeBreakdown, 'total'), 'count'));
    } elseif (isset($clientSexBreakdownOctoberToDecember)) {
        $totalForms = array_sum(array_map(function ($counts) {
            return $counts['Internal'] + $counts['External'];
        }, $clientSexBreakdownOctoberToDecember));
    } elseif (isset($q4MunicipalityBreakdown)) {
        $totalForms = array_sum(array_column(array_column($q4MunicipalityBreakdown, 'total'), 'count'));
    } elseif (isset($q4CategoryBreakdown)) {
        $totalForms = array_sum(array_column(array_column($q4CategoryBreakdown, 'total'), 'count'));
    }
@endphp
...
<div class="card" style="margin-top: 20px; margin-left: 20px; border-radius: 10px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); background-color: #fff;">
    ...
    <div class="card-body">
        <table class="table table-bordered">
            ...
            <tbody>
                @foreach ($q4AgeBreakdown as $ageRange => $data)
                    <tr>
                        <td>{{ $ageRange }}</td>
                ...
                <tr>
                    ...
                    <td>
                        <strong>
                            {{ array_sum(array_column(array_column($q4AgeBreakdown, 'external'), 'count')) }} 
                            ({{ number_format(array_sum(array_column(array_column($q4AgeBreakdown, 'external'), 'percentage')), 2) }}%)
                        </strong>
                    </td>
                    <td>
                        <strong>
                            {{ array_sum(array_column(array_column($q4AgeBreakdown, 'internal'), 'count')) }} 
                            ({{ number_format(array_sum(array_column(array_column($q4AgeBreakdown, 'internal'), 'percentage')), 2) }}%)
                        </strong>
                    </td>
                    <td>
                        <strong>
                            {{ array_sum(array_column(array_column($q4AgeBreakdown, 'total'), 'count')) }} 
                            ({{ number_format(array_sum(array_column(array_column($q4AgeBreakdown, 'total'), 'percentage')), 2) }}%)
                        </strong>
                    </td>
                </tr>
            </tbody>
    ...
    <div class="card-body">
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                @foreach ($q4MunicipalityBreakdown as $municipality => $counts)
                    @php
    ...
    <div class="card-body">
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                @foreach ($q4CategoryBreakdown as $category => $counts)
                    @php
