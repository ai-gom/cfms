    ...
    <div class="card-body">
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                @foreach ($octoberToDecemberData['ageRanges'] as $range => $data)
                    <tr>
                        <td>{{ $range }}</td>
    ...
    <div class="card-body">
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                <tr>
                    <td>Male</td>
                    <td>{{ number_format($octoberToDecemberData['maleExternalPercentage'], 2) }}%</td>
                    <td>{{ number_format($octoberToDecemberData['maleInternalPercentage'], 2) }}%</td>
                    <td>{{ number_format($octoberToDecemberData['maleOverallPercentage'], 2) }}%</td>
                </tr>
                @php
                    $totalExternalPercentage += $octoberToDecemberData['maleExternalPercentage'];
                    $totalInternalPercentage += $octoberToDecemberData['maleInternalPercentage'];
                    $totalOverallPercentage += $octoberToDecemberData ['maleOverallPercentage'];
                @endphp

                <!-- Female Row -->
                <tr>
                    <td>Female</td>
                    <td>{{ number_format($octoberToDecemberData['femaleExternalPercentage'], 2) }}%</td>
                    <td>{{ number_format($octoberToDecemberData['femaleInternalPercentage'], 2) }}%</td>
                    <td>{{ number_format($octoberToDecemberData['femaleOverallPercentage'], 2) }}%</td>
                </tr>
                @php
                    $totalExternalPercentage += $octoberToDecemberData['femaleExternalPercentage'];
                    $totalInternalPercentage += $octoberToDecemberData['femaleInternalPercentage'];
                    $totalOverallPercentage  += $octoberToDecemberData['femaleOverallPercentage'];
                @endphp

                <!-- Prefer Not to Say Row -->
                <tr>
                    <td>Prefer Not to Say</td>
                    <td>{{ number_format($octoberToDecemberData['preferNotToSayExternalPercentage'], 2) }}%</td>
                    <td>{{ number_format($octoberToDecemberData['preferNotToSayInternalPercentage'], 2) }}%</td>
                    <td>{{ number_format($octoberToDecemberData['preferNotToSayOverallPercentage'], 2) }}%</td>
                </tr>
                @php
                    $totalExternalPercentage += $octoberToDecemberData['preferNotToSayExternalPercentage'];
                    $totalInternalPercentage += $octoberToDecemberData['preferNotToSayInternalPercentage'];
                    $totalOverallPercentage +=  $octoberToDecemberData['preferNotToSayOverallPercentage'];
                @endphp
    ...
    <div class="card-body">
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                @foreach ($octoberToDecemberData['municipalityData'] as $municipality => $data)
                    <tr>
                        <td>{{ $municipality }}</td>
                        <td>{{ $data['external']['count'] }} ({{ number_format($data['external']['percentage'], 2) }}%)</td>
                        <td>{{ $data['internal']['count'] }} ({{ number_format($data['internal']['percentage'], 2) }}%)</td>
                ...
                @php
                    ...
                    $totalForms = $octoberToDecemberData['totalForms'];
                    $totalExternalPercentage = ($totalForms > 0) ? ($totalExternalCount / $totalForms) * 100 : 0;
                    $totalInternalPercentage = ($totalForms > 0) ? ($totalInternalCount / $totalForms) * 100 : 0;
                    $totalOverallPercentage = ($totalForms > 0) ? ($totalOverallCount / $totalForms) * 100 : 0;
                @endphp
    ...
    <div class="card-body">
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                @foreach ($octoberToDecemberData['clientCategories'] as $category => $data)
                    <tr>
                        <td>{{ $category }}</td>
                        <td>{{ $data['external']['count'] }} ({{ number_format($data['external']['percentage'], 2) }}%)</td>
                        <td>{{ $data['internal']['count'] }} ({{ number_format($data['internal']['percentage'], 2) }}%)</td>
...
<!-- Card for Citizen's Charter Awareness - Oct to Dec -->
<div class="card" style="margin-top: 20px; margin-left: 20px; border-radius: 10px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); background-color: #fff;">
                    <div class="card-header">
                        ...
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                @foreach(['cc1', 'cc2', 'cc3'] as $ccKey)
                                    ...
                                    @foreach($octoberToDecemberResponses[$ccKey] as $option => $data)
                                        <tr>
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                <tr>
                    ...
                    <td>{{ $octoberToDecemberTotals['strongly_agree'] }}</td>
                    <td>{{ $octoberToDecemberTotals['agree'] }}</td>
                    <td>{{ $octoberToDecemberTotals['neither'] }}</td>
                    <td>{{ $octoberToDecemberTotals['disagree'] }}</td>
                    <td>{{ $octoberToDecemberTotals['strongly_disagree'] }}</td>
                    <td>{{ $octoberToDecemberTotals['na'] }}</td>
                    <td>{{ $octoberToDecemberTotals['total_responses'] }}</td>
                    <td>{{ number_format($octoberToDecemberTotals['average_overall_score'], 2) }}%</td>
                </tr>
            </tbody>
        </table>
    </div>
    ...
    <div class="card-body">
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                @endphp
                @foreach($octoberToDecemberExpectationsBreakdown as $field => $breakdown)
                    @php
    ...
    <div class="card-body" style="overflow-x: auto;">
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                @foreach( $serviceAveragesQ4 as $service)
